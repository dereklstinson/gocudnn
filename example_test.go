...
func ExampleTensorD() {
	...
	//Make an Allocator
	CudaMemManager, err := cudart.CreateMemManager(dev) //cs could be nil .  Check out cudart package on more about streams
	check(err)
...
func ExampleStream() {
	...
	dev, err := cudart.CreateDevice(1)
	check(err)
	CudaMemManager, err := cudart.CreateMemManager(dev)
	check(err)
